...
$(document).on('click.radio.data-api','[data-toggle^=radio], .radio',function(e){var $radio=$(e.target);if(e.target.tagName!="A"){e&&e.preventDefault()&&e.stopPropagation();if(!$radio.hasClass('radio'))$radio=$radio.closest('.radio');$radio.find(':radio').radio('toggle');}});$(function(){$('[data-toggle="radio"]').each(function(){var $radio=$(this);$radio.radio();});});}(window.jQuery);
;
...
var img={croppa:function(src,width,height,options){if(!src)return;if(!width)width='_';if(!height)height='_';var suffix='-'+width+'x'+height;if(options&&options instanceof Array){var val,key;for(key in options){val=options[key];if(val instanceof String)suffix+='-'+val;else if(val instanceof Object){for(key in val){val=val[key];if(val instanceof Array)suffix+='-'+key+'('+val.join(',')+')';else suffix+='-'+key+'('+val+')';break;}}}}
return src.replace(/^(.+)(\.[a-z]+)$/i,"$1"+suffix+"$2");}};
(function($){$(function(){var $window,$body,$document,$halfWidth,$halfHeight;$window=$(window);$body=$('body');$document=$(document);$halfHeight=$window.height()/2;$halfWidth=$window.width()/2;$('img.quiz-img').each(function(index,image)
{var path=$(this).attr('src'),width,height;if($window.width()<=320){height=$window.height()/3;width=$window.width()/2;}else{height=$window.height()/3;width=$window.width()/4;}
$(this).attr('src',(img.croppa(path,Math.round(width),Math.round(height))));});$(':radio').on('change',function(){console.log($(this));});$(':radio').on('toggle',function(){console.log($(this));});$(document).on('click','.remote-load-trigger',function(event)
{var $url=$(this).data('src');console.log($url);$(document).find('#content').load($url,function(response)
{$(this).fadeIn(response);$(':radio').radio();});event.preventDefault()});$(document).on('change','input[name="answer"]',function(){if($(this).val()===$('input[name="correctAnswer"]').val()){$(this).closest('li').find('.status').css('background-color','#16a085');$('.remote-load-trigger').click();}else{$(this).closest('li').find('.status').css('background-color','#e74c3c');}});});})(jQuery);